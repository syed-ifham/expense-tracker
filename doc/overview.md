...
**Smart Financial Tracker** is a Spring Boot backend that automatically parses and tracks financial transactions from SMS sourceMessages. It ingests sourceMessages from a local SQLite database (YourPhone cache), processes them through multiple parsing strategies, and provides RESTful APIs.
...
|---------|---------------|---------|
| **Strategy Pattern** | `SmsParsingStrategy` interface + 3 concrete implementations | Extensible sourceMessage parsing for different bank formats |
...
// GET /
{ "status": "OK", "sourceMessage": "I am OK" }
```
